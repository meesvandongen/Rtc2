...
- 726b2dc: Lay the toolbar out yourself with `toolbarLayout`.
...
- 32db075: Centre the empty and error states across the whole table in the `grid` layout
  modes.
...
- 7c29751: Add `transposed`: the table with its axes flipped, one row per column and one
  column per record.
  ...
  empty _row_ between the two sections, turned. It takes the whole gap the records
  ...
  _column_ window for the same reason they would have to: each spans the whole
...
- af608e9: Draw the edge of a pinned block as a border rather than a shadow.
...
- 2bd718a: Put pinned rows where pinning promised they would be.
