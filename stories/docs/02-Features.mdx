...
| Row pinning | `enableRowPinning` (bool or predicate) — pin/unpin from each row's overflow menu, `rowPinningDisplayMode` (`sticky` \| `top` \| `bottom` \| `top-and-bottom`), `keepPinnedRows`¹ — a pinned row survives the filter or page that drops it |
