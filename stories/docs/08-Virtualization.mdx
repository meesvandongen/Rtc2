...
`rowPinningDisplayMode: 'sticky'` — which elsewhere leaves a pinned row in
place and holds it against both edges — renders the top and bottom sections
here instead. They sit outside the virtualized body and hold their own
position, so a pinned row stays on screen; what it gives up is its place in the
order, which absolute positioning had already taken.
