...
`rowPinningDisplayMode: 'sticky'` — which elsewhere keeps pinned rows in the
body's own rows, at its two ends — renders the top and bottom sections here
instead. They sit outside the virtualized body and hold their own position,
which is the same edge the rows would have held anyway.
